...
void Display() {
    if(!closing && renderer && game) { game->Draw(*renderer); glutSwapBuffers(); }
}
void Resize(int w,int h) { if(renderer) renderer->Resize(w,h); }
void Close() {
    closing=true;
    if(game && !game->Save()) std::cerr << "Closing without a new save; previous save retained.\n";
    game.reset();
...
void Tick(int) {
    ...
    game->Update(dt,focused&&(Down('W')||Down(VK_UP)),focused&&(Down('S')||Down(VK_DOWN)),
                 focused&&(Down('A')||Down(VK_LEFT)),focused&&(Down('D')||Down(VK_RIGHT)),
...
void Key(unsigned char key,int,int) {
    ...
    if(key==27 && !game->IsDeathPrompt()) {
        if(game->Save()) glutLeaveMainLoop();
        return;
    }
    game->Action(key);
}
